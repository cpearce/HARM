...
void Apriori(Options& options) {
  ...
  InvertedDataSetIndex index(options.inputFileName.c_str());
  index.Load();
...
void Apriori_Test() {
  {
    ...
    // Test GenerateInitialCandidates().
    {
      ...
      ASSERT(CheckSet("a s u", candidates));
    }
    {
      set<ItemSet> candidates;
    ...
    // Test bool ContainsAllSubSets(const set<ItemSet> &aContainer, const ItemSet &aItemSet)
    {
      ...
      ItemSet abc("a", "b", "c");
      ASSERT(ContainsAllSubSets(container, abc));
    }
    // Test GenerateCandidates()...
    {
      ...
      ASSERT(IntersectionSize(ItemSet("1", "2", "3"), ItemSet("1", "2", "4")) == 2);
      ASSERT(IntersectionSize(ItemSet("1", "2", "4"), ItemSet("1", "3", "4")) == 2);
      ASSERT(IntersectionSize(ItemSet("1", "3", "4"), ItemSet("1", "3", "5")) == 2);
      ASSERT(IntersectionSize(ItemSet("1", "2", "3"), ItemSet("2", "3", "4")) == 2);
      ...
      ASSERT(R.size() == 1);
      ASSERT((*R.begin()) == ItemSet("1", "2", "3", "4"));
    }
  }
  {
    ...
    */
    int c = 0;
    ASSERT((c = index.Count(ItemSet("1"))) == 2);
    ASSERT((c = index.Count(ItemSet("2"))) == 3);
    ASSERT((c = index.Count(ItemSet("3"))) == 3);
    ASSERT((c = index.Count(ItemSet("5"))) == 3);

    MinCountFilter filter(2, index);
    set<ItemSet> initialCandidates;
    GenerateInitialCandidates(index, filter, initialCandidates);
    ASSERT(CheckSet("1 2 3 5", initialCandidates));

    //Test next generation...
    set<ItemSet> nextGen;
    GenerateCandidates(initialCandidates, nextGen, 2, &filter, 1);
    ASSERT(CheckSet("1,3 2,3 2,5 3,5", nextGen));

    set<ItemSet> tmp = nextGen;
    nextGen.clear();
    GenerateCandidates(tmp, nextGen, 3, &filter, 1);
    ASSERT(CheckSet("2,3,5", nextGen));
