...
void CoocurrenceGraph::Test() {
  ...
  ASSERT((c = g.CoocurrenceCount(Item("a"), Item("b"))) == 5);
  ASSERT((c = g.CoocurrenceCount(Item("a"), Item("c"))) == 2);
  ASSERT((c = g.CoocurrenceCount(Item("c"), Item("b"))) == 1);
  ASSERT((c = g.CoocurrenceCount(Item("c"), Item("d"))) == 2);
  ASSERT((c = g.CoocurrenceCount(Item("a"), Item("d"))) == 0);

  ASSERT((c = g.CoocurrenceCount(Item("h"), Item("j"))) == 0);

  g.Increment(ItemSet("h", "j", "k").AsVector());
  g.Increment(ItemSet("a", "c", "k").AsVector());

  ASSERT((c = g.CoocurrenceCount(Item("h"), Item("j"))) == 1);
  ASSERT((c = g.CoocurrenceCount(Item("j"), Item("k"))) == 1);
  ASSERT((c = g.CoocurrenceCount(Item("h"), Item("k"))) == 1);
  ASSERT((c = g.CoocurrenceCount(Item("a"), Item("k"))) == 1);
  ASSERT((c = g.CoocurrenceCount(Item("a"), Item("c"))) == 3);

  g.Decrement(ItemSet("h", "j", "k").AsVector());

  ASSERT((c = g.CoocurrenceCount(Item("h"), Item("j"))) == 0);
  ASSERT((c = g.CoocurrenceCount(Item("j"), Item("k"))) == 0);
  ASSERT((c = g.CoocurrenceCount(Item("h"), Item("k"))) == 0);
  ASSERT((c = g.CoocurrenceCount(Item("a"), Item("k"))) == 1);
  ASSERT(g.CoocurrenceCount(Item("a"), Item("c")) == 3);
  ASSERT(g.CoocurrenceCount(Item("c"), Item("k")) == 1);

  vector<Item> n;
  g.GetNeighbourhood(Item("a"), n);
  ASSERT(ItemSet("b", "c", "k") == n);

  g.Decrement(ItemSet("a", "c", "k").AsVector());
  ASSERT((c = g.CoocurrenceCount(Item("a"), Item("k"))) == 0);
  ASSERT(g.CoocurrenceCount(Item("a"), Item("c")) == 2);
  ASSERT(g.CoocurrenceCount(Item("c"), Item("k")) == 0);

  n.clear();
  g.GetNeighbourhood(Item("a"), n);
  ASSERT(ItemSet("b", "c") == n);
